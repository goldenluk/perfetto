...
size_t HttpServer::ParseOneHttpRequest(HttpServerConnection* conn) {
  ...
  // Checked before the body is requested, so that a request that is about to
  // be refused is never handed a payload sink. OPTIONS is exempt: it is the
  // preflight itself, answered by HandleCorsPreflightRequest() below.
