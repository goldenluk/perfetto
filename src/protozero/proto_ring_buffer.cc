...
RingBufferMessageReader::WriteHandle::~WriteHandle() {
  PERFETTO_CHECK(reader_ == nullptr);  // Must be consumed or moved from.
}
...
void RingBufferMessageReader::WriteHandle::EndWrite(size_t size_written) {
  ...
  RingBufferMessageReader* reader = reader_;
  reader_ = nullptr;
  reader->FinishWrite(size_written);
}
