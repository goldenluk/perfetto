 ...
 public:
  ...
  std::optional<lineage::analysis::LeafRelation> FindLeafRelation(
    ...
    for (const TestColumn& column : dataframe->second) {
      relation.columns.push_back(column.name);
    }
    return relation;
  }
  ...
  std::optional<core::StorageType> ColumnType(
      const lineage::analysis::ColumnLineage& column) const override {
    std::optional<core::StorageType> type;
    for (const auto& origin : column.origins) {
      auto dataframe = dataframes_.find(std::string(origin.relation_name));
      if (dataframe == dataframes_.end()) {
        return std::nullopt;
      }
      auto found =
          std::find_if(dataframe->second.begin(), dataframe->second.end(),
                       [&](const TestColumn& candidate) {
                         return candidate.name == origin.column_name;
                       });
      if (found == dataframe->second.end() ||
