...
class Rpc {
 public:
  ...
  //
  // The TraceProcessor instance behind them is shared by all streams; what a
  // Stream owns is the framing state, which is not shareable: the tokenizer
  // holding a message that arrived in pieces, and the request/response
  // sequence ids, which each peer numbers from its own 0. Interleaving the
  // bytes of two peers into one Stream therefore corrupts both, which is why
  // this is per-connection rather than per-Rpc.
  //
  // Note that this isolates framing only. Streams share one TraceProcessor,
  // so two peers can still trip over each other's state (e.g. one resetting
  // the trace while the other queries it); this class draws no session
  // boundary and is not meant to.
  class Stream {
   public:
    ...
    // A reservation inside this stream's tokenizer for the transport to read
    // into, so that it never needs a staging buffer of its own (e.g. by
    // handing data() to read(2)). Move-only and must be consumed exactly once:
    // EndRequest() reports how many bytes were written and dispatches whatever
    // messages completed, AbortRequest() discards it. Letting one go out of
    // scope unconsumed fails a CHECK.
    class RequestHandle {
     public:
     ...
     private:
      friend class Stream;
      RequestHandle(Stream* stream,
                    protozero::ProtoRingBuffer::WriteHandle write)
  ...
  // Takes the bytes as they are, without copying them.
