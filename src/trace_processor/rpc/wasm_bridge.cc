...
Rpc::Stream::RequestHandle* g_pending_request;

// Size of the standing write reservation, chosen by JS in the init call.
...
uint8_t* trace_processor_rpc_init(RpcResponseFn* resp_function,
  ...
  g_pending_request = new Rpc::Stream::RequestHandle(
      g_rpc_stream->BeginRequest(g_max_write_size));
  return g_pending_request->data();
}

// Closes the write JS just made into the address we last handed it and returns
// where the next one goes. The standing reservation is what lets that address
// be known before its size is, keeping this to one JS->Wasm call per request.
