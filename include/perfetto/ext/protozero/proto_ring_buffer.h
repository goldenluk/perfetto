...
class ProtoRingBuffer {
 public:
  ...
  // Must be consumed exactly once, by EndWrite() to keep what was written or
  // AbortWrite() to discard it; dropping one fails a CHECK. The buffer will
  // not recompact, grow or hand out a second while one is outstanding.
  class WriteHandle {
   public:
    WriteHandle() = default;
    ~WriteHandle();
